...
const recalculateLayout = (moduleId: string, modules: Module[], excelData: ExcelProduct[] | null): Module[] => {
  ...
  // Znajdź produkty tego modułu w excelData
  const moduleProducts = module.products.map(product => {
    ...
    return {
      ...product,
      position: excelProduct?.position || 0
    };
  });

  // POPRAWIONE GRUPOWANIE: Produkty grupowane w "kolumny" według części całkowitej pozycji
  // Przykład: 1.1 i 1.2 → kolumna 1, 2.0 → kolumna 2, 3.0 → kolumna 3
  const groups: { [key: number]: Product[] } = {};
  moduleProducts.forEach(product => {
    const groupKey = Math.floor(product.position || 0);
  ...
  let currentX = 0; // Zaczynamy od lewej krawędzi (x: 0)
  ...
  sortedGroupKeys.forEach(groupKey => {
    ...
    // POZYCJONOWANIE PIONOWE: Sortowanie produktów w kolumnie według części dziesiętnej pozycji
    // Tylko produkty w tej samej kolumnie (ta sama część całkowita) układają się w pionie
    ...
    // Oblicz szerokość grupy (najszerszy produkt w stosie)
    const groupWidth = Math.max(...sortedGroupProducts.map(p => p.scaledWidth));

    // POZYCJONOWANIE PIONOWE: Wyrównanie do dolnej krawędzi modułu
    // Produkty w tej samej kolumnie układają się jeden na drugim
    let stackHeight = 0;
    const productsWithY = sortedGroupProducts.map((product, index) => {
      let yPosition;
      
      if (index === 0) {
        // Pierwszy produkt (bazowy) - dolna krawędź przy dnie modułu
        yPosition = module.height - product.scaledHeight;
        stackHeight = product.scaledHeight;
      } else {
        // Kolejne produkty układane na poprzednim
      ...
      return {
        ...product,
        x: currentX, // POZYCJONOWANIE POZIOME: Wszystkie produkty w kolumnie mają tę samą pozycję X
        y: Math.max(0, yPosition) // Zapobieganie ujemnym wartościom Y
      };
    });

    updatedProducts.push(...productsWithY);
    currentX += groupWidth; // POZYCJONOWANIE POZIOME: Przejście do następnej kolumny (bez odstępu)
  });

  // Aktualizuj moduł z nowymi pozycjami produktów
